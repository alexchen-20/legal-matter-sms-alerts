...
> Design notes for **legal-matter-sms-alerts** — a runnable python example that Typed Python service for matter intake, signed document, and deadline SMS alerts.
...
- **Thin client** — a ~30-line helper that owns the base URL, the auth header, and envelope unwrapping, so call sites stay readable (e.g. `infrai.sms.send(...)`).
- **Feature code** — the actual task: legal-matter-alerts.
- **Configuration** — the API key is read from the `INFRAI_API_KEY` environment variable; no secret is ever hard-coded.

## Capabilities used

- `sms.send` — mapped to `POST /v1/sms/send`.
